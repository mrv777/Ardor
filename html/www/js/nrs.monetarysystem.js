...
NRS.onSiteBuildDone().then(() => {
    NRS = (function (NRS, $, undefined) {
            ...
            if (response.currencies && response.currencies.length || response.accountCurrencies && response.accountCurrencies.length) {
                ...
                if (NRS.currenciesPageType == "my_currencies") {
                    if (requestType == "searchCurrencies+") {
                        ...
                        let accountCurrencies = await NRS.sendRequestAndWait("getAccountCurrencies+", {
                            account: NRS.accountRS,
                        ...
                        if (accountCurrencies.currency) {
                            ...
                            accountCurrencies["accountCurrencies"].push(accountCurrencies);
                            rows = getAccountCurrenciesRows(accountCurrencies);
                        }
                    } else {
        ...
        distributionModal.on("show.bs.modal", async function (e) {
            let $invoker = $(e.relatedTarget);
            let code = $invoker.data("code");
            ...
            NRS.sendRequest("getCurrencyAccounts", {
                ...
                if (response.accountCurrencies) {
                    ...
                    for (let i = 0; i < response.accountCurrencies.length; i++) {
                        let account = response.accountCurrencies[i];
                        let percentageCurrency = NRS.calculatePercentage(account.unitsQNT, currency.currentSupplyQNT);
