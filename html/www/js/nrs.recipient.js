	...
	NRS = (function(NRS, $) {
		NRS.automaticallyCheckRecipient = function() {
			var $recipientFields = $("#send_money_recipient, #transfer_asset_recipient, #transfer_currency_recipient, " +
				...
				"#add_monitored_account_recipient, #grant_permission_recipient, #remove_permission_recipient");
		...
		function appendAccount($list, account) {
		...
		$allModals.on("click", "span.recipient_selector button, span.plain_address_selector button", function(e) {
			...
			$list.empty();
			for (let accountId in NRS.contacts) {
		...
		NRS.checkRecipient = async function(account, modal) {
			...
			if (NRS.isRsAccount(account)) {
				...
				if (address.set(account)) {
					NRS.checkAccountStatus(account, function(response) {
						...
						if (address.account_id() === NRS.account) {
							callout.removeClass(classes).addClass("callout-" + response.type).html($.t("this_is_your_account")).show();
						} else {
		...
		NRS.getAccountAlias = async function(alias) {
			let result = {};
			...
			if (response.aliasURI) {
				...
				let regex_1 = /acct:(.*)@nxt/;
				let regex_2 = /nacc:(.*)/;
				let match = aliasURI.match(regex_1);
				if (!match) {
		...
		NRS.checkRecipientAlias = async function(alias, modal) {
			var classes = "callout-info callout-danger callout-warning";
			var callout = modal.find(".account_info").first();
			var accountInputField = modal.find("input[name=converted_account_id]");
			accountInputField.val("");
			var result = await NRS.getAccountAlias(alias);
			if (result.error) {
