	...
	NRS = (function(NRS, $) {
		...
		NRS.pages.contacts = function() {
			...
			NRS.storageSelect("contacts", null, function (error, contacts) {
				...
				NRS.setupChainWarning($("a[data-target=\\#send_message_modal]"), false);
			});
		};

		NRS.forms.addContact = async function($modal) {
			...
			let response = await NRS.sendRequestAndWait("getAccount", {
		...
		NRS.forms.updateContact = async function($modal) {
			var data = NRS.getFormData($modal.find("form:first"));
			data.account_id = String(data.account_id);
			if (!data.name) {
				return {
