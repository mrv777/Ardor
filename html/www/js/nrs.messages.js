...
NRS.onSiteBuildDone().then(() => {
	NRS = (function(NRS, $) {
		...
		const $messagesSidebar = $("#messages_sidebar");
		...
		NRS.pages.messages = function(callback) {
			...
			NRS.sendRequest("getBlockchainTransactions+", {
				...
				if (response.transactions && response.transactions.length) {
					...
					NRS.appendSmallScreenSidebarReplacementSelect($("#messages_page"), (sidebarItem) => {
						let address = $(sidebarItem).find('h4').text();
						let timestamp = $(sidebarItem).find('.list-group-item-text').text();
						if (address.length > 8) {
		...
		NRS.jsondata.messages = async function (response) {
			...
			var decoded = await getMessage(response);
			var decryptAction = "";
			...
			if (!decryptAction && !retrieveAction && decoded.hash && decoded.message == $.t("binary_data")) {
				downloadAction = NRS.getMessageDownloadLink(response.fullHash, decoded.sharedKey);
			}
			return {
		...
		function displayMessageSidebar(callback) {
			...
			var messagesSidebar = $messagesSidebar;
		...
		async function getMessage(message) {
			...
			if (!message.attachment) {
				...
				try {
					$.extend(decoded, await NRS.tryToDecryptMessage(message));
				...
				let response = await NRS.sendRequestAndWait("getPrunableMessage", { transactionFullHash: message.fullHash, retrieve: "false"});
				if (response.errorCode || !response.transactionFullHash) {
					...
					decoded = await getMessage(message);
				}
			} else {
				decoded.message = $.t("message_empty");
			}
			if (!NRS.isEmptyObject(decoded)) {
				if (!decoded.message) {
					decoded.message = $.t("message_empty");
				}
		...
		$messagesSidebar.on("click", "a", async function(e) {
			e.preventDefault();
			$messagesSidebar.find("a.active").removeClass("active");
			...
			if (messages) {
				for (var i = 0; i < messages.length; i++) {
					var decoded = await getMessage(messages[i]);
			...
			const $sendMessageSubmit = $("#inline_message_submit");
			$sendMessageSubmit.data("account", NRS.convertNumericToRSAccountFormat(otherUser));
		});
		...
		$("#inline_message_form").submit(async function(e) {
			e.preventDefault();
		});
		...
		NRS.forms.sendMessageComplete = function(response, data) {
			...
			const splitter = $('#messages_page').find('.content-splitter-right-inner');
			splitter.scrollTop(splitter[0].scrollHeight);
		};
		...
		NRS.forms.decryptMessages = function($modal) {
			...
			if (data.rememberPassword) {
				NRS.setDecryptionPrivateKey(privateKey);
			}
			$messagesSidebar.find("a.active").trigger("click");
			if (success) {
		...
		$("#send_message_modal").on("show.bs.modal", function (e) {
			const sendMessageMessage = $("#send_message_message");
			const $invoker = $(e.relatedTarget);
			const $inlineMessageSubmit = $("#inline_message_submit");
			sendMessageMessage.prop("readonly", false);
			if ($invoker.is($inlineMessageSubmit)) {
				copyMessageFields();
			} else {
		...
		function copyMessageFields() {
			const $sendMessageMessage = $("#send_message_message");
			const $inlineMessageText = $("#inline_message_text");
			$sendMessageMessage.prop("value", $inlineMessageText.val());

			const $inlineMessagePassword = $("#inline_message_password");
			const $sendMessagePassword = $("#send_message_password");
			$sendMessagePassword.prop("value", $inlineMessagePassword.val());

			const $encryptMessage = $("#send_message_modal_true_encrypt_message");
			const $inlineEncrypt = $("#inline_message_true_encrypt_message");
			$encryptMessage.prop("checked", $inlineEncrypt.prop("checked"));

			const $notencryptMessage = $("#send_message_modal_false_encrypt_message");
			const $inlineNotEncrypt = $("#inline_message_false_encrypt_message");
			$notencryptMessage.prop("checked", $inlineNotEncrypt.prop("checked"));
		}
