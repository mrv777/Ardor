...
NRS.onSiteBuildDone().then(() => {
    NRS = (function (NRS, $, undefined) {
        ...
        $("#buy_alias_modal").on("show.bs.modal", async function(e) {
            let $modal = $(this);
            let $invoker = $(e.relatedTarget);
            let alias = String($invoker.data("alias"));
        ...
        $("#register_alias_modal").on("show.bs.modal", async function (e) {
            var $invoker = $(e.relatedTarget);
        ...
        NRS.forms.setAliasError = async function (response, data) {
            if (response && response.errorCode && response.errorCode == 8) {
                let errorDescription = NRS.escapeRespStr(response.errorDescription);
                let response = await NRS.sendRequestAndWait("getAlias", {
