...
NRS.onSiteBuildDone().then(() => {
	NRS = (function(NRS, $) {
		...
		$body.on('click', '.modal div[data-modal-ui-element="multi_piece_modal_ui_element"] .combine-pieces-btn', function() {
			...
			var account = NRS.getAccountId(privateKey);
			var isVoucher = $(this).closest("form").find("input[name=isVoucher]").prop("checked");
			if (account !== NRS.account && !isVoucher) {
				$(this).closest(".modal").find(".error_message").html($.t("error_passphrase_incorrect_v2", { account: NRS.convertNumericToRSAccountFormat(account) })).show();
				return;
			}
