...
QUnit.module("nrs.ledger.comparison");

const MNEMONIC2 = "van library enough young maximum jelly solar elder rose omit journey violin lazy someone second stay scale mosquito tennis peasant brand stay blouse almost";
...
QUnit.test("ledgerComparison", function (assert) {
    let mainnetPath = "m/44'/16754'/0/0";
    for (let i = 1; i < MAINNET_DATA.length; i++) {
        let parentNode = KeyDerivation.deriveMnemonic(mainnetPath, MNEMONIC2);
        let childNode = KeyDerivation.deriveChildPrivateKey(parentNode, i);
        assert.equal(MAINNET_DATA[i][0], converters.byteArrayToHexString(childNode.getPublicKey()));
        assert.equal(MAINNET_DATA[i][1], converters.byteArrayToHexString(childNode.getMasterPublicKey()));
        assert.equal(MAINNET_DATA[i][2], converters.byteArrayToHexString(childNode.getChainCode()));
        assert.equal(MAINNET_DATA[i][3].substring(0, 64), converters.byteArrayToHexString(childNode.getPrivateKeyLeft()));
        assert.equal(MAINNET_DATA[i][3].substring(64, 128), converters.byteArrayToHexString(childNode.getPrivateKeyRight()));

        let childPublicKeyNode = KeyDerivation.deriveChildPublicKeyFromNode(parentNode, i);
        let curve25519PublicKey = childPublicKeyNode.getPublicKey();
        assert.equal(MAINNET_DATA[i][0], converters.byteArrayToHexString(curve25519PublicKey));
        assert.equal(MAINNET_DATA[i][1], converters.byteArrayToHexString(childPublicKeyNode.getMasterPublicKey()));

        let publicKey = curve25519.keygen(childNode.getPrivateKeyLeft()).p;
        assert.equal(converters.byteArrayToHexString(curve25519PublicKey), converters.byteArrayToHexString(publicKey));
    }
});
