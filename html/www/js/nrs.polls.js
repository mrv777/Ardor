	...
	NRS = (function(NRS, $, undefined) {
		...
		body.on("click", ".vote_button[data-poll]", function(e) {
			...
			NRS.sendRequest("getPoll", {
				...
				$(".answer_slider input[type='range']").on("change mousemove", function() {
		...
		NRS.loadFollowedPollsSidebar = function(callback) {
			...
			NRS.appendSmallScreenSidebarReplacementSelect($("#followed_polls_page"), (sidebarItem) => {
				let name = $(sidebarItem).find('h4').text();
				let status = $(sidebarItem).find('.list-group-item-text').text();
				return `${name} - ${status}`;
			});
