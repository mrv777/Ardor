    ...
    NRS = (function(NRS, $) {
        ...
        NRS.jsondata.shuffling = function (response, shufflers, amountDecimals) {
            ...
            return {
                status:
                    (function () {
                        if (response.stage === 1 && response.issuer !== NRS.account || response.stage > 1) {
        ...
        NRS.jsondata.standbyshuffler = function (response, minAmountDecimals, maxAmountDecimals) {
            return {
                ...
                publicKeysLeft: isBip32StandbyShuffler(response) ? '<i class="far fa-infinity"></i>' : NRS.escapeRespStr(response.recipientPublicKeys.length),
                holdingType: NRS.escapeRespStr(response.holdingType),
                holding: NRS.escapeRespStr(response.holding),
                reservedPublicKeys: isBip32StandbyShuffler(response) ? "" : NRS.escapeRespStr(response.reservedPublicKeysCount),
                index: isBip32StandbyShuffler(response) ? NRS.escapeRespStr(response.currentDerivationInfoChildIndex) : ""
            }
        };

        function isBip32StandbyShuffler(standbyShufflerResponse) {
            return standbyShufflerResponse.currentDerivationInfoChildIndex !== undefined;
        }
        ...
        NRS.setup.shuffling = function() {
            ...
            NRS.appendMenuItemToTSMenuItem(sidebarId, {
                "titleHTML": '<span data-i18n="standby_shufflers">Standby Shufflers</span>',
                "type": 'PAGE',
                "page": 'standby_shufflers'
            });
            NRS.appendMenuItemToTSMenuItem(sidebarId, {
        ...
        $('#m_shuffler_start_recipient_secretphrase').on("change", function () {
            ...
            let privateKey = NRS.getPrivateKey(secretPhraseValue);
            recipientAccount.val(NRS.getAccountId(privateKey, true));
        });
        ...
        $('#m_start_standbyshuffler_recipient_secretphrases').on('change', function () {
            ...
            var accounts = convertSecretPhrasesList(secretPhrases, function (secretPhrase) {
                ...
                return NRS.getAccountId(privateKey, true);
            });
            recipientAccounts.val(accounts);
        });

        $('#m_start_standbyshuffler_modal .tab_selector_public_key_origin li').on('click', function (ev) {
            ev.preventDefault();
            const $li = $(this);
            const origin = $li.data('origin');
            $li.addClass('active').siblings().removeClass('active');
            $li.parents('form')
                .find('.public_key_origin').addClass('hidden').end()
                .find('.' + origin).removeClass('hidden');
        });

        NRS.forms.startShuffler = function ($modal) {
            var data = NRS.getFormData($modal.find("form:first"));
            if (data.recipientSecretPhrase) {
                data.recipientPublicKey = NRS.getPublicKeyFromSecretPhrase(data.recipientSecretPhrase);
        ...
        NRS.forms.startStandbyShuffler = function ($modal) {
            ...
            const publicKeyOrigin = $('#m_start_standbyshuffler_modal .tab_selector_public_key_origin li.active').data('origin');

            if (publicKeyOrigin === 'passphraseList') {
                delete data.serializedMasterPublicKey;
                delete data.startFromChildIndex;

                // read directly from the text area since the form data returns \r\n as line separator in some configurations
                const recipientSecretPhrases = $('#m_start_standbyshuffler_recipient_secretphrases').val();
                if (recipientSecretPhrases) {
                    data.recipientPublicKeys = convertSecretPhrasesList(recipientSecretPhrases,
                        secretPhrase => NRS.getPublicKeyFromSecretPhrase(secretPhrase));
                }
            }
