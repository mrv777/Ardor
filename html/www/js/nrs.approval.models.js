...
NRS.onSiteBuildDone().then(() => {
    NRS = (function(NRS, $) {
        ...
        NRS.setApprovalModelsPrivateKey = function(privateKey) {
        ...
        function loadApprovalModels() {
            ...
            if (!NRS.isJSONObjectEmpty(approvalModels)) {
                Object.keys(approvalModels).sort().forEach(function(key) {
                    let name = String(key).escapeHTML();
                    var approvalModel = approvalModels[key];
                    var description = NRS.escapeRespStr(approvalModel.description).nl2br();
        ...
        $("#approval_model_modal a.at_hash").on('shown.bs.tab', () => {
            if (NRS.rememberPassword) {
                $('#add_approval_model_password').val(sessionPrivateKey).change();
            }
        });

        NRS.forms.addApprovalModel = async function($modal) {
            ...
            if (data.phasingVotingModel == NRS.constants.VOTING_MODELS.COMPOSITE) {
                let response = await NRS.sendRequestAndWait("evaluateExpression", { expression: data.phasingExpression });
                if (response.errorCode) {
                    let error = NRS.unescapeRespStr(response.errorDescription);
                    if (response.subPoll) {
            ...
            let response = await NRS.sendRequestAndWait("parsePhasingParams", data);
            if (response.errorCode) {
                let error = NRS.unescapeRespStr(response.errorDescription);
                if (response.subPoll) {
                    error += " " + $.t("for_sub_poll") + " " + response.subPoll;
        ...
        NRS.forms.importAssetControl = async function($modal) {
            let data = NRS.getFormData($modal.find("form:first"));
            let response = await NRS.sendRequestAndWait("getPhasingAssetControl", { "asset": data.id });
            let model = {
                "ASC": $.extend({
        ...
        $("#add_approval_model_password").change(function() {
            let privateKey = NRS.getPrivateKey($(this).val());
            if (NRS.account !== NRS.getAccountId(privateKey)) {
                $.growl($.t("generate_secret_wrong_passphrase", { account: NRS.accountRS } ));
                return;
            }
            NRS.generateSecret(privateKey, function (result) {
                if (result.error) {
                    $.growl(result.error)
                } else {
        ...
        $("#reproduce_secret_modal").on("show.bs.modal", function(e) {
            ...
            if (NRS.rememberPassword) {
                $('#reproduce_secret_password').val(sessionPrivateKey).change();
            }
        ...
        $("#reproduce_secret_password").change(function() {
            var privateKey = NRS.getPrivateKey($("#reproduce_secret_password").val());
            var calculatedSecret = NRS.calculateSecret(privateKey, $("#reproduce_secret_nonce").val(), $("#reproduce_secret_block_id").val());
            $("#reproduce_secret_generated_secret").val(calculatedSecret);
            var $modal = $("#reproduce_secret_modal");
            hashSecret(calculatedSecret, NRS.constants.PHASING_HASH_ALGORITHMS[$("#reproduce_secret_hash_algorithm").val()], function (hash) {
