...
public enum CurrencyType {
    ...
    RESERVABLE(0x04) {
        ...
        void validate(Currency currency, Transaction transaction, Set<CurrencyType> validators) throws NxtException.ValidationException {
            ...
            if (transaction.getType() == MonetarySystemTransactionType.RESERVE_INCREASE) {
                ...
                if (currency.getIssuanceHeight() <= attachment.getFinishValidationHeight(transaction)) {
    ...
    static void validateCurrencyNaming(ChildTransaction transaction, CurrencyIssuanceAttachment attachment) throws NxtException.ValidationException {
        ...
        if (code.equals(childChain.getName().toUpperCase(Locale.ROOT)) || childChain.getName().toLowerCase(Locale.ROOT).equals(normalizedName)) {
            throw new NxtException.NotValidException("Currency name already used");
        }
