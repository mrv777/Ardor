...
public final class VoteWeighting {
    ...
    public enum VotingModel {
        ...
        COMPOSITE(6) {
            @Override
            public long calcWeight(VoteWeighting voteWeighting, long voterId, int height) {
        ...
        public static final int MIN_CODE = Arrays.stream(values()).mapToInt(VotingModel::getCode).min().getAsInt();
        public static final int MAX_CODE = Arrays.stream(values()).mapToInt(VotingModel::getCode).max().getAsInt();
