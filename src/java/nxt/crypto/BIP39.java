 ...
 */
public class BIP39 {
    public static final String[] ENGLISH_WORDLIST = {"abandon", "ability", "able", "about", "above", "absent",
    ...
    private static final Map<String, Integer> BIP39_EN_WORDS_MAP = IntStream.range(0, ENGLISH_WORDLIST.length).boxed()
            .collect(Collectors.toMap(i -> ENGLISH_WORDLIST[i], i -> i));

    private BIP39() {}

    /**
     * Computes back the source entropy given a BIP39 mnemonic.
     *
     * The mnemonic checksum is verified. It uses the english wordlist.
     *
     * @param mnemonic the BIP39 mnemonic as a list of words
     * @return the entropy corresponding to the mnemonic
     */
    static BigInteger mnemonicToEntropy(String[] mnemonic) {
        if (mnemonic.length % 3 != 0) {
            throw new IllegalArgumentException("Invalid number of words: " + mnemonic.length);
        }
        int[] wordIndexes = findWordIndexes(mnemonic);
        int ms = wordIndexes.length;
        int entPlusCs = ms * 11;
        int ent = (entPlusCs * 32) / 33;
        int cs = ent / 32;
        if (ent < 128 || ent > 256 || entPlusCs != ent + cs) {
            throw new IllegalArgumentException("Invalid number of words: " + ms);
        }

        BigInteger bits = BigInteger.ZERO;
        for (int wordIndex : wordIndexes) {
            bits = bits.shiftLeft(11);
            bits = bits.add(BigInteger.valueOf(wordIndex));
        }
        BigInteger entropy = bits.shiftRight(cs);
        BigInteger checksum = bits.and(BigInteger.ONE.shiftLeft(cs).subtract(BigInteger.ONE));
        int computedChecksum = computeChecksum(entropy, ent);

        if (checksum.intValueExact() != computedChecksum) {
    ...
    /**
     * Generates a BIP39 mnemonic given an entropy and the number of bits of the entropy.
     *
     * We need two parameters because we are using BigInteger as the storage for the entropy instead
     * of, for example, a byte array. This is for easier integration with the SecretSharingGenerator.
     *
     * It uses the BIP39 english word list.
     *
     * @param entropy some random entropy
     * @param numberOfBits the number of bits of the entropy, must be divisible by 32 and between 128 and 256
     * @return the BIP39 mnemonic as a list of words
     */
    static String[] entropyToMnemonic(BigInteger entropy, int numberOfBits) {
        if (numberOfBits % 32 != 0 || numberOfBits < 128 || numberOfBits > 256) {
            throw new IllegalArgumentException("Incorrect number of entropy bits: " + numberOfBits);
        }
        int cs = numberOfBits / 32;
        int ms = (numberOfBits + cs) / 11;
        int checksum = computeChecksum(entropy, numberOfBits);

        BigInteger bits = entropy.shiftLeft(cs).add(BigInteger.valueOf(checksum));
        String[] mnemonic = new String[ms];
        BigInteger mask = BigInteger.valueOf(2047);
        for(int i = ms - 1; i >= 0; i--) {
            int index = bits.and(mask).intValueExact();
            mnemonic[i] = ENGLISH_WORDLIST[index];
     ...
     *
     * @param mnemonic the list of words comprising the mnemonic
     * @return true if the given list of words is a valid BIP39 mnemonic
     */
    ...
    /**
     * Returns the amount of entropy in bits given the number of BIP39 words.
     *
     * @param numberOfWords number of BIP39 words
     * @return the number of entropy bits
     */
    static int getEntropyBits(int numberOfWords) {
        if (numberOfWords % 3 != 0 || numberOfWords < 12 || numberOfWords > 24) {
            throw new IllegalArgumentException("Invalid number of words: " + numberOfWords);
        }
        return (numberOfWords * 11 * 32) / 33;
    }

    private static int computeChecksum(BigInteger entropy, int numberOfBits) {
        // we need to compute the SHA256 of the entropy bits but we need the exact amount of bits with leading zeros
        byte[] entropyArray = Convert.parseHexString(Convert.bigIntegerToHexString(entropy, numberOfBits / 8 * 2));
        byte[] sha256 = Crypto.sha256().digest(entropyArray);
        return Byte.toUnsignedInt(sha256[0]) >>> (8 - numberOfBits / 32);
    }

    private static int[] findWordIndexes(String[] mnemonic) {
        int[] indexes = new int[mnemonic.length];
        for (int i = 0; i < mnemonic.length; i++) {
            if (!BIP39_EN_WORDS_MAP.containsKey(mnemonic[i])) {
                throw new IllegalArgumentException("Invalid BIP39 english word: " + mnemonic[i]);
            }
            indexes[i] = BIP39_EN_WORDS_MAP.get(mnemonic[i]);
        }
        return indexes;
    }
}
